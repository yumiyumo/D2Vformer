...
class TD_encoderlayer(nn.Module):
    """
    Autoformer encoder layer with the progressive decomposition architecture
    """
    def __init__(self, attention, d_model, d_ff=None, moving_avg=25, dropout=0.1, activation="relu"):
        super(TD_encoderlayer, self).__init__()
        d_ff = d_ff or 4 * d_model
...
class TD_decoderlayer(nn.Module):
    """
    Autoformer decoder layer with the progressive decomposition architecture
    """
    def __init__(self, self_attention, cross_attention, d_model, c_out, d_ff=None,
                 moving_avg=25, dropout=0.1, activation="relu"):
        super(TD_decoderlayer, self).__init__()
        d_ff = d_ff or 4 * d_model
        self.self_attention = self_attention
        self.cross_attention = cross_attention
        self.conv1 = nn.Conv1d(in_channels=d_model, out_channels=d_ff, kernel_size=1, bias=False)
...
class Trend_process(nn.Module):
    ...
    def forward(self, x, x_mask=None, cross_mask=None):
        ...
        B,L,D = x.shape
        theta_trend = self.trend_block(x.permute(0,2,1))
        t = self.linear_space(L)
        trend_part = self.trend_model(theta_trend,t,x.device).permute(0,2,1)

        if self.norm is not None:
            trend_part = self.norm(trend_part)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, cross_s, x_mask=None, cross_mask=None, trend=None):
        for layer in self.layers:
            x, residual_trend = layer(x, cross_s, x_mask=x_mask, cross_mask=cross_mask)
            if trend != None:
