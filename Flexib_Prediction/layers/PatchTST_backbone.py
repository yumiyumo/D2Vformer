...
class PatchTST_backbone(nn.Module):
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        # model
        z = self.backbone(
            z)  # 模型类似transofmrer，后面dim与bs通道合并，意味着每个对象的通道独立，通道信息没有融合  输出维度：z[bs,dim,hidden_dimension,patch_num]
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    ...
    def forward(self, x) -> Tensor:  # x: [bs ,nvars , patch_len , patch_num]
        ...
        u = torch.reshape(x, (
        x.shape[0] * x.shape[1], x.shape[2], x.shape[3]))  # 合并bs和dim维度       # u: [bs * nvars , patch_num , d_model]
        u = self.dropout(u + self.W_pos)  # 加入位置编码信息                       # u: [bs * nvars , patch_num , d_model]

        # Encoder
        z = self.encoder(u)  # z: [bs * nvars ,patch_num , d_model]
        z = torch.reshape(z, (
        -1, n_vars, z.shape[-2], z.shape[-1]))  # 取消bs和dim维度的合并              # z: [bs x nvars , patch_num , d_model]
